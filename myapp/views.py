...
class Paytm(View):
	def get(self,request):
		param_dict = {
                'MID': 'BNZBVO38003525155031',
                'ORDER_ID': '123',
                'TXN_AMOUNT': '1234',
                'CUST_ID': 'aftab4evr786@gmail.com',
                'INDUSTRY_TYPE_ID': 'Retail',
                'WEBSITE': 'WEBSTAGING',
                'CHANNEL_ID': 'WEB',
                'CALLBACK_URL':'http://127.0.0.1:8000/handel-request/',

        }
		param_dict['CHECKSUMHASH'] = Checksum.generate_checksum(param_dict, 'd7CXkBKR40jK9NDu')
...
def HandlePaytmrequest(request):
	...
	for i in form.keys():
		...
		if i == 'CHECKSUMHASH':
			print("i: ",i)
			checksum = form[i]
	verify = Checksum.verify_checksum(response_dict, 'd7CXkBKR40jK9NDu', checksum)
	if verify:
